...
\author{bess-stack-de \\ Degradation-aware rolling-horizon dispatch for a
grid-connected BESS in the German market}
\date{V1.0}
...
\texttt{./src/bess\_stack/model/dispatch.py}. The project models
...
\texttt{solve\_window}; no equation there describes intended behaviour. Section 3
formulates the reserve products from the parameters already present in
\texttt{scenarios/reference.yaml}, none of which the code reads today. The
...
\texttt{scenarios/reference.yaml}; those beginning with a stream name
(\texttt{day\_ahead}, \texttt{fcr}, \texttt{afrr}) are relative to
\texttt{revenue\_streams}.

\small
\begin{tabular}{@{}l p{4.6cm} l p{5.4cm}@{}}
\toprule
Symbol & Description & Unit & Source field \\
\midrule
$\Delta t$   & Length of one time step & h & \texttt{market.resolution\_minutes} \\
$\pi_t$      & Day-ahead price in step $t$ & EUR/MWh & price series argument \\
$\kappa$     & Spread retention, $1 - \text{haircut}$ & -- & \texttt{day\_ahead.spread\_haircut} \\
$f$          & Trading fee per MWh traded & EUR/MWh & \texttt{day\_ahead.fees\_eur\_per\_mwh} \\
$g^{c}$      & Network fee on energy charged & EUR/MWh & \texttt{grid.charging\_fees\_eur\_per\_mwh} \\
$g^{d}$      & Network fee on energy discharged & EUR/MWh & \texttt{grid.discharging\_fees\_eur\_per\_mwh} \\
$c^{\deg}$   & Marginal degradation cost & EUR/MWh & \texttt{degradation.marginal\_cost\_eur\_per\_mwh} \\
$\bar{P}$    & Binding power cap & MW & $\min$(\texttt{battery.power\_mw}, \texttt{grid.connection\_limit\_mw}) \\
$E^{\text{nom}}$ & Nominal usable energy & MWh & \texttt{battery.energy\_mwh} \\
$\eta^{c}$   & One-way charging efficiency & -- & \texttt{battery.efficiency.charge} \\
$\eta^{d}$   & One-way discharging efficiency & -- & \texttt{battery.efficiency.discharge} \\
$\sigma$     & Self-discharge, fraction of $E^{\text{nom}}$ per day & 1/d & \texttt{battery.efficiency.self\_discharge\_per\_day} \\
$P^{\text{aux}}$ & Parasitic auxiliary load & MW & \texttt{battery.aux\_load\_mw} \\
$L$          & Idle energy loss per step, see (2.5) & MWh & derived \\
$E^{\min}$   & Lower energy bound & MWh & \texttt{battery.soc.min} $\times\, E^{\text{nom}}$ \\
$E^{\max}$   & Upper energy bound & MWh & \texttt{battery.soc.max} $\times\, E^{\text{nom}}$ \\
$E_{\text{init}}$ & Energy at window start & MWh & carried between windows \\
$E^{\text{term}}$ & Required energy at window end & MWh & \texttt{dispatch.terminal\_soc\_fraction} $\times\, E^{\text{nom}}$ \\
$m$          & Time steps per day-ahead block & -- & \texttt{day\_ahead.product\_resolution\_minutes} \\
$H$          & Optimization horizon & h & \texttt{dispatch.horizon\_hours} \\
$C$          & Committed step of the rolling horizon & h & \texttt{dispatch.commit\_step\_hours} \\
\bottomrule
\end{tabular}
\normalsize
...
\begin{itemize}
  ...
  \item $c^{\deg}$ --- the marginal cost of degradation, charged per MWh
        ...
        \]
        where $\phi$ is \texttt{degradation.cyclic\_fade\_per\_full\_cycle}
        and $\kappa^{\text{aug}}$ is
        \texttt{degradation.augmentation.cost\_eur\_per\_kwh}. Nominal energy
        ...
        exactly that. \texttt{config.py} enforces the bound. There is no
...
Setting \texttt{dispatch.mode} to \texttt{perfect\_foresight} collapses the
...
the rolling mode while \texttt{dispatch.forecast.kind} remains \texttt{perfect}:
...
\texttt{./src/bess\_stack/finance/grid\_fees.py} to an already-solved dispatch,
...
present in \texttt{scenarios/reference.yaml}, but \texttt{config.py} defines no
FCR or aFRR dataclass and does not read them, and \texttt{solve\_window} builds
...
\midrule
$c^{\text{FCR}}$      & FCR capacity price (EUR/MW/h) & \texttt{fcr.capacity\_price\_eur\_per\_mw\_h} \\
$h^{\text{FCR}}$      & Energy headroom held per MW awarded (h) & \texttt{fcr.reserve\_energy\_hours} \\
$R^{\min}$            & Minimum bid size (MW) & \texttt{fcr.min\_bid\_mw}, \texttt{afrr.min\_bid\_mw} \\
$c^{a\pm}$            & aFRR capacity price, each direction & \texttt{afrr.\{positive,negative\}.capacity\_price\_eur\_per\_mw\_h} \\
$\pi^{a\pm}$          & aFRR energy price, each direction & \texttt{afrr.\{positive,negative\}.energy\_price\_eur\_per\_mwh} \\
$\alpha^{\pm}$        & Expected activation rate & \texttt{afrr.\{positive,negative\}.activation\_rate} \\
$h^{a}$               & aFRR energy headroom per MW awarded (h) & \texttt{afrr.reserve\_energy\_hours} \\
\bottomrule
\end{tabular}
\normalsize
